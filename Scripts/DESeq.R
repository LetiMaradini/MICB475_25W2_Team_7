...
res_LAR_pairs <- purrr::map_dfr(seq_along(time_comparisons), function(i) {
  ...
  if (cmp$use_time) {
    # Time-based: filter Time + treatment_group
    ...
    # Week9 subtypes: filter sample_type_group directly
    keep <- sample_data(micro_plus1)$sample_type_group %in% cmp$groups
    comp_label <- paste(cmp$groups, collapse = " vs ")
  }
  
  if (sum(keep) < 4) {  # Need ~2 per group minimum
    warning(paste("Skipping", comp_label, "- only", sum(keep), "samples"))
    return(data.frame())
  }
  
  micro_sub <- prune_samples(keep, micro_plus1)
  if (nsamples(micro_sub) == 0) return(data.frame())
...
res_LAR_pairs %>% 
  filter(comparison == "F-LAR vs GC-LAR") %>% 
  volcano_plot("vol_plot_F-LAR_vs_GC-LAR_0.png")
res_LAR_pairs %>% 
  filter(comparison == "F-LAR vs GC-LAR") %>%  # Week4.5 same label
...
# Split LAR_pairs by comparison 
lar_combined <- res_LAR_pairs %>% filter(comparison == "F-LAR vs GC-LAR")
half_size <- nrow(lar_combined) / 2
...
sig_summary <- tibble(
  ...
  res_obj = list(
    ...
    # === DYNAMIC - Matches volcano filters ===
    lar_combined %>% slice_head(n = half_size),           # 0w half
    lar_combined %>% slice_tail(n = half_size),            # 4.5w half  
  ...
  ungroup() %>%
  select(comparison, tested, up, down, ns) %>%
  write_csv("ASVs_count_summary.csv")
print(sig_summary)

#### Generate sig ASV barplots + save tables for each comparison ####

# Prepare combined LAR 0w/4.5w split
lar_combined <- res_LAR_pairs %>% filter(comparison == "F-LAR vs GC-LAR")
half_size <- nrow(lar_combined) / 2
...
sig_plots <- tibble(
  ...
  res_obj = list(
    ...
    res_GCLAR_45_vs_0, res_GCLAR_9fresh_vs_0, res_GCLAR_9necro_vs_0, 
    res_GCLAR_9fresh_vs_45, res_GCLAR_9necro_vs_45, res_GCLAR_9fresh_vs_9necro,
    lar_combined %>% slice_head(n = half_size),
    lar_combined %>% slice_tail(n = half_size),
    res_LAR_pairs %>% filter(comparison == "F-LAR-Fresh vs GC-LAR-Fresh"),
    res_LAR_pairs %>% filter(comparison == "F-LAR-Necropsy vs GC-LAR-Necropsy"),
    res_return_flight, res_controls_9, res_ctrl_basal
  )
  ...
  rowwise() %>%
  filter(sum(res_obj$padj < 0.05 & abs(res_obj$log2FoldChange) > 2, na.rm = TRUE) > 0) %>%
  mutate(
    sig_table = list({
      strict_sig <- res_obj %>% 
        filter(padj < 0.05 & abs(log2FoldChange) > 2, !is.na(padj))
      sigASVs_vec <- strict_sig %>% pull(row)
      print(paste(comparison, "strict sig:", nrow(strict_sig)))
      
      tax_df <- tax_table(prune_taxa(sigASVs_vec, micro_final)) %>% 
        as.data.frame() %>% rownames_to_column("ASV")
      strict_sig_clean <- strict_sig %>% mutate(ASV = row) %>% filter(!is.na(ASV))
      tax_df %>%
    ...
    plot = list({
      ...
      print(p)
      ggsave(paste0("sig_ASVs_", gsub("[^A-Za-z0-9]", "_", comparison), ".png"),
             p, width = 12, height = 6)
      p
    }),
...
# Write tables outside mutate
purrr::walk2(sig_plots$sig_table, sig_plots$filename, readr::write_csv)

print(paste(
  "Generated", nrow(sig_plots), "plots + tables:",
  paste(sig_plots$filename, collapse = ", ")
))
