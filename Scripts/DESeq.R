...
volcano_plot_pub_ready <- function(res, filename, point_size = 2) {
  dat <- res %>%
    mutate(
      log_padj = ifelse(is.na(padj), NA, ifelse(padj == 0, 300, -log10(padj)))
    )
  
  labelled_dat <- dat %>%
    mutate(ASVs = case_when(
      !is.na(padj) & padj < 0.05 & log2FoldChange > 2 ~ "Enriched",
      !is.na(padj) & padj < 0.05 & log2FoldChange < -2 ~ "Depleted",
  ...
  p <- labelled_dat %>%
    ggplot(aes(x = log2FoldChange, y = log_padj, color = ASVs)) +
    geom_point(size = point_size) +
    geom_hline(yintercept = -log10(0.05), linetype = "dashed", linewidth = 0.8) +
    geom_vline(xintercept = c(-2, 2), linetype = "dashed", linewidth = 0.8) +
  ...
  print(p)
  ggsave(filename, p, width = 7, height = 7, dpi = 300)
  return(p)
}

p1 <- volcano_plot_pub_ready(res_microgravity, "vol_plot_F-ISST_vs_GC-ISST_9.png")
p2 <- volcano_plot_pub_ready(
  res_LAR_pairs %>% filter(comparison == "Launch plus 0 | F-LAR vs GC-LAR"),
  "vol_plot_F-LAR_vs_GC-LAR_0.png"
)
p3 <- volcano_plot_pub_ready(
  res_LAR_pairs %>% filter(comparison == "Launch plus 4.5 | F-LAR vs GC-LAR"),
  "vol_plot_F-LAR_vs_GC-LAR_4.5.png"
)
p4 <- volcano_plot_pub_ready(
  res_LAR_pairs %>% filter(comparison == "F-LAR-Fresh vs GC-LAR-Fresh"),
  "vol_plot_F-LAR-Fresh_vs_GC-LAR-Fresh.png"
)
p5 <- volcano_plot_pub_ready(
  res_LAR_pairs %>% filter(comparison == "F-LAR-Necropsy vs GC-LAR-Necropsy"),
...
fig_ABCDE <- (p1 | p2) /
  ...
  theme(
    ...
    legend.title = element_text(size = 23, face = "plain"),
...
print(fig_ABCDE)
ggsave("volcano_panels_ABCDE.jpg", fig_ABCDE, width = 16, height = 18, dpi = 300)